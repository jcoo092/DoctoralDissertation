...
\newacronym{dpsm}{DPSM}{dynamic~programming \glsxtrshort{sm}}
...
\newacronym{gpgpu}{GPGPU}{general-purpose \glsxtrlong{gpu} computing}
\newacronym{lbp}{LBP}{loopy \glsxtrlong{bp}}
\newacronym{mpbsm}{MPBSM}{message passing-based \glsxtrshort{sm}}
\newacronym{mrf}{MRF}{Markov random field}
\newacronym{mwt}{MWT}{moving window transform}

%% N-Z
\newacronym{ndcsm}{NDCSM}{noise-driven concurrent \glsxtrshort{sm}}
...
% A-M
\newdefacr{cml}{CML}{Concurrent~ML}{Concurrent~ML}{A programming style introduced by John Reppy as an extension of the pre-existing programming language Standard~ML.  Conceptually, Concurrent~ML has a lot in common with \glsxtrlong{csp} \cite{Hoare1985}.  Both involve every logical process mostly advancing on their own, but sometimes synchronising with others over channels (see also \vref{fig:back:cml_exchange}).  Concurrent~ML is described in detail in \cite{Reppy2007}.}
