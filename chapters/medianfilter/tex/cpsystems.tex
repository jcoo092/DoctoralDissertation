\section{Median Filtering with \glsentrytext{cps}}\label{sec:medianfilter}
% \cpresetrulenumber
...
\Cref{ruleset:medianfilter} sets out the rules, building on the selection rules of \vref{rules:cps:selectmultisetid} in \cref{sec:cps:selectmultisetid}.  They are integrated with the counting rules of \vref{rules:cps:countelems} in \cref{sec:cps:countelems} to find the median index.
...
% \Cref{ruleset:medianfilter} sets out the rules, which build on Ruleset~\ref{rules:cps:selectmultisetid} in Section~\ref{sec:cps:selectmultisetid}.  They are integrated with the counting rules to find the median index.   \cpRuleref{rule:medianfilter:antiport} receives, through antiport exchange, messages from every connected neighbour and stores the values received while sending the current pixel's value back.  \cpRuleref{rule:medianfilter:storeown} stores the pixel's own value in the \(s\) functor alongside the messages received from neighbours since it is one of the values to consider here.  It runs once messages have been received from all neighbours.
...
\cptermdef{b}{Pixel value.  Holds the current pixel's own value, to exchange with neighbours and consider later in the median filtering process.}
...
\section{\glsentrytext{cps} Example}
...
% \begin{tabular}{|r|l|}
% \hline
% \textbf{Step} & \textbf{Objects in the pixel \gls{tlc}} \\ \hline
...
% \end{tabular}
% \caption[Objects present inside the \gls{tlc} at the end of each step of median filtering]{Objects present inside the \gls{tlc} at the end of each step}
...
\end{cpobjects}
\caption{\label{objs:medianfilter:ex0}Objects inside an arbitrary \gls{tlc} at the end of step 0 of the median filtering process.}
\end{cpobjectsfloat}
...
% \cpobjectsline{\mathbf{\cpfunc{a}{\cpempty}} \quad \mathbf{\cpfunc{c}{4}}\) \(\cpfuncn{s}{2}{8}\) \(\cpfuncn{s}{3}{3}\) \(\cpfuncn{s}{4}{6}\) \(\cpfuncn{s}{5}{3}\)  \(\cpfuncnn{t}{2}{8}{1}\) \(\cpfuncnn{t}{3}{3}{1}\)\\& \(\cpfuncnn{t}{4}{6}{1}\) \(\cpfuncnn{t}{5}{3}{1}}
\end{cpobjects}
\caption{\label{objs:medianfilter:ex1}Objects inside an arbitrary \gls{tlc} at the end of step 1 of the median filtering process.}
\end{cpobjectsfloat}
...
% \cpobjectsline{\cpfunc{a}{\cpempty}\)} \sout{\(\cpfunc{b}{6}\)} \(\mathbf{\cpfunc{c}{5}}\) \(\cpfuncn{s}{1}{6}\) \(\cpfuncnn{t}{1}{6}{1}}
\end{cpobjects}
\caption{\label{objs:medianfilter:ex2}Objects inside an arbitrary \gls{tlc} at the end of step 2 of the median filtering process.}
\end{cpobjectsfloat}
...
\end{cpobjects}
\caption{\label{objs:medianfilter:ex3}Objects inside an arbitrary \gls{tlc} at the end of step 3 of the median filtering process.}
\end{cpobjectsfloat}
...
\end{cpobjects}
\caption{\label{objs:medianfilter:ex4}Objects inside an arbitrary \gls{tlc} at the end of step 4 of the median filtering process.}
\end{cpobjectsfloat}
...
% Assume that an arbitrary exemplar pixel has four neighbours, represented in \(\cpfunc{a}{\cpfunc{n}{1} \, \cpfunc{n}{2} \, \cpfunc{n}{3} \, \cpfunc{n}{4}}\), and its value is 6, \(\cpfunc{b}{6}\).  The objects present in the \gls{tlc} at the end of every step are listed in \cref{tab:exampleobjects}, following the format of the examples in \cref{sec:cps:stats}.  When \cpruleref{rule:medianfilter:antiport} is applied, the pixel gains four new \(s\) and four new \(t\) functors, \(\cpfuncn{s}{2}{8}\) \& \(\cpfuncnn{t}{2}{8}{1}\), \(\cpfuncn{s}{3}{3}\) \& \(\cpfuncnn{t}{3}{3}{1}\), \(\cpfuncn{s}{4}{6}\) \& \(\cpfuncnn{t}{4}{6}{1}\), \(\cpfuncn{s}{5}{3}\) \& \(\cpfuncnn{t}{5}{3}{1}\).  \(\cpfunc{c}{0}\) becomes \(\cpfunc{c}{4}\).  \cpRuleref{rule:medianfilter:storeown} adds \(\cpfuncn{s}{1}{6}\) \& \(\cpfuncnn{t}{1}{6}{1}\), and \(\cpfunc{c}{4}\) becomes \(\cpfunc{c}{5}\).

Assume that an arbitrary exemplar pixel has four neighbours, represented in \(\cpfunc{a}{\cpfunc{n}{1} \, \cpfunc{n}{2} \, \cpfunc{n}{3} \, \cpfunc{n}{4}}\), and its value is 6, \(\cpfunc{b}{6}\).  The objects present in the \gls{tlc} at the end of every step are listed in \crefrange{objs:medianfilter:ex0}{objs:medianfilter:ex5}, following the format of the examples in \vref{sec:cps:stats}.  When \cpruleref{rule:medianfilter:antiport} is applied, the pixel gains four new \(s\) and four new \(t\) functors, \(\cpfuncn{s}{2}{8}\) \& \(\cpfuncnn{t}{2}{8}{1}\), \(\cpfuncn{s}{3}{3}\) \& \(\cpfuncnn{t}{3}{3}{1}\), \(\cpfuncn{s}{4}{6}\) \& \(\cpfuncnn{t}{4}{6}{1}\), \(\cpfuncn{s}{5}{3}\) \& \(\cpfuncnn{t}{5}{3}{1}\).  \(\cpfunc{c}{0}\) becomes \(\cpfunc{c}{4}\).  \cpRuleref{rule:medianfilter:storeown} adds \(\cpfuncn{s}{1}{6}\) \& \(\cpfuncnn{t}{1}{6}{1}\), and \(\cpfunc{c}{4}\) becomes \(\cpfunc{c}{5}\).
