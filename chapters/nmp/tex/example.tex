...
The total trace for this example is: \tracn{{\label{trace:0}}--}{--}{--} \tarr{} \tracn{\label{trace:1}0?*}{0?*}{0?*} \tarr{} \tracn{\label{trace:2}1?*}{0}{1?} \tarr{} \tracn{\label{trace:3}1}{0}{2?} \tarr{} \tracn{\label{trace:4}2?}{1?**}{2} \tarr{} \tracn{\label{trace:5}2}{2?}{2} \tarr{} \tracn{\label{trace:6}--}{--}{--}.  The state of Node 1 at the end of each round is shown in \autoref{tab:nmp:exampleobjects}.  The round number for each row in the table matches the round number described further below.  Where the contents of a functor change between the end of one round and the end of the next, the functor and its changed contents are written in boldface to highlight the modifications.
...
\end{tabular}
\caption[Objects present inside Node 1 at the end of each round]{Objects present inside Node 1 at the end of each round in the example.  Functors whose contents have changed from one step to the next, and the changed contents themselves, are marked in boldface.}
