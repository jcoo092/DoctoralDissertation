...
% \begin{proof}
%     By \cpruleref{rule:nmp:proxspec:recvinputs}, \gls{pe} $\pi_0$ starts by accepting a value $\cpvv{k}{0}{\cpdiscard}$, from the environment channel $e$.
...
% Thus, by part (ii), each \gls{pe} $\pi_k$ sends a generation $g+1$ value $v$,
...
Across all 10 rules, two atoms, eight functors, and eight states are used.  This gives a total symbol complexity of 18 distinct symbols.  As with the rule complexity, there likely will be more symbols used when the \gls{oq} phase is expanded to encompass the desired computation.
