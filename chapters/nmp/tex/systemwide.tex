...
% The system's evolution is sketched at a high-level in \autoref{alg:nmp:systemwide2}.  Lines 3 and 4 represent the initialisation phase and correspond to rules 1 and 2, respectively.  The \texttt{while} loop spanning lines 5-10 encompasses both the \gls{oq} and \gls{nm} phases.  The loop test on line 5 corresponds to rule 3.  Rule 4 matches line 6 and rule 5 with line 7.  Combined, those two are the \gls{oq} phase.   Rules 6 and 7 match with lines 9 and 10, respectively, and are the \gls{nm} phase.  Lastly, lines 12 and 13, respectively, reflect rules 8 and 9, comprising the finalisation phase.
...
\begin{cprulesetfloat}
    ...
    \end{cpruleset}
    \caption{\label{ruleset:nmp:systemwide}Complete ruleset for synchronous \gls{nmp}, using an oracle to perform update computations}
\end{cprulesetfloat}
...
\begin{description}
    ...
    \cpterm{\cpempty}{The \gls{cps} `empty' atom (see \autoref{sec:lr:cpsystems}).}
\end{description}
...
\begin{description}
    ...
    \cpterm{v}{The \(v\) compound terms described in \autoref{sec:nmp:compoundterms}, which serve as \gls{nm} messages, \emph{except} here we elide the generation counter because it is unneeded --- the \(i\) counter serves the same purpose.}
