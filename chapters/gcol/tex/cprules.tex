...
\caption[Pseudocode representation of the algorithm performed by the cP~system rules in \autoref{ruleset:gcol:rules}]{\label{code:gcol:graphcol}Pseudocode representation of the algorithm performed by the cP~system rules in \autoref{ruleset:gcol:rules}.  We use a doubly defined tail-recursive function approach, where the algorithm begins with the upper function supplied only with the details of the graph and colours, but uses the lower function with further arguments to perform most of the processing.}
\end{algorithm}
