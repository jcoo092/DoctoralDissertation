...
\end{cpobjects}
\caption[Example set of objects inside the \gls{tlc} after the second step for the \gls{gcp} \gls{cps} solution]{\label{objs:gcol:obj2}Set of objects inside the \gls{tlc} after the second step (\ie{} after one application of rules \cpruleref*{rule:gcol:rules:loop} and \cpruleref*{rule:gcol:rules:loopclean}) for \cref{fig:gcol:examplegraph}.}
\end{cpobjectsfloat}
