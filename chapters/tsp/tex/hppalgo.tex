\section{\label{sec:tsp:algohpp}\glsentrytext{cps} Hamiltonian Path and Cycle Algorithms}
...
This \namecref{sec:tsp:algohpp} discusses only digraphs, since all undirected graphs can be represented quite simply as a directed graph with arcs in both directions simulating edges --- this is in fact how undirected graphs are represented in the below algorithm.

\subsection{Formal System Definition}
% A \glspl{cps} can be described as a 6-tuple, as shown below.
...
\subsection{\label{sec:tsp:hpp}Hamiltonian Path}
...
\begin{description}
    ...
    \cptermdef{h}{List head.  Represents the head of a classic \fxwarning{ref}{\texttt{cons} linked list}, which is used to track a path explored-so-far in the digraph.}
